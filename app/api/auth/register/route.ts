...
export async function POST(request: Request) {
  ...
  if (!limiter.allowed) {
    return fail("RATE_LIMITED", "Too many registration attempts. Try again shortly.", 429)
  }
  ...
  try {
    ...
    if (!parsed.success) {
      return fail("VALIDATION_ERROR", "Invalid registration data", 400, parsed.error.flatten())
    }
